...
pub const AsyncTaskManaged = struct {
    const ExecuteOnly = struct {
        pub fn execute(arena: *std.heap.ArenaAllocator, result: f64) !f64 {
            _ = try arena.allocator().dupe(u8, "don't leave me hanging!");
            return result;
        }
    };
    ...
    pub fn scheduleExecuteOnly(call: t.Call, result: f64) !t.Promise {
        const allo = dba.allocator();
        return call.env.asyncTaskManaged(allo, ExecuteOnly, .{result});
    }

    const WithComplete = struct {
        pub fn execute(arena: *std.heap.ArenaAllocator, result: f64) !f64 {
            _ = try arena.allocator().dupe(u8, "clean up on aisle 5!");
            return result;
        }

        pub fn complete(_: t.Env, result: f64) ![]const u8 {
            var buf: [8]u8 = undefined;
            return std.fmt.bufPrint(&buf, "{d}", .{result});
        }
    };
    ...
    pub fn scheduleWithComplete(call: t.Call, result: f64) !t.Promise {
        const allo = dba.allocator();
        return call.env.asyncTaskManaged(allo, WithComplete, .{result});
    }

    const WithCleanup = struct {
        pub fn execute(arena: *std.heap.ArenaAllocator, result: f64) !f64 {
            _ = try arena.allocator().dupe(u8, "i won't be here long...right?");
            return result;
        }

        var on_cleanup_ref: ?t.Ref(t.Fn) = null;

        pub fn cleanUp(env: t.Env) !void {
            const cb_ref = on_cleanup_ref orelse {
                return error.MissingCleanupCallbackRef;
            };

            const on_cleanup = try cb_ref.val(env) orelse {
                return error.BrokenCleanupCallbackRef;
            };
    ...
    pub fn scheduleWithCleanup(
        call: t.Call,
        result: f64,
        on_cleanup: t.Fn,
    ) !t.Promise {
        WithCleanup.on_cleanup_ref = try on_cleanup.ref(1);

        return call.env.asyncTaskManaged(dba.allocator(), WithCleanup, .{
            result,
        });
    }
    ...
    const WithErrConvert = struct {
        ...
        pub fn execute(arena: *std.heap.ArenaAllocator) !f64 {
            _ = try arena.allocator().dupe(u8, "allo, allo, de-allo. mahalo.");

            return error.WrongWay;
        }

        pub fn errConvert(_: t.Env, err: anyerror) ![]const u8 {
            return @errorName(err);
        }
    ...
    pub fn scheduleWithErrConvert(call: t.Call, on_cleanup: t.Fn) !t.Promise {
        WithErrConvert.on_cleanup_ref = try on_cleanup.ref(1);

        return call.env.asyncTaskManaged(dba.allocator(), WithErrConvert, .{});
    }
};
