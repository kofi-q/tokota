...
/// Retrieves native data previously attached to the JS value via
/// `Env.external()`.
///
/// Returns `Err`.`InvalidArg` for non-external JS values or those with
/// different external value types. The latter is enforced using `Object.Tag`s.
///
